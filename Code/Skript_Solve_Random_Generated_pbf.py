...
if __name__ == "__main__":
    ...
    print("Offset_increase="+str(Offset_increase))
    ...
    print("variables = " + str(variables))
    print("QUBO2")
    pbf_neg = createPoly_negative(variables,degree,1,seed=seed_gen)
    pbf_neg=Sort_pbf(pbf_neg,2)
    pbf_var_dict_neg = createPolyDict(pbf_neg,variables) 
    print("feature generation time:" + str(time.time() - start_time_gen))
    for t in range(20):
        
        print("# Test auf Temepratur:" + str(1.5+0.1*(t)))
        cooling_param = ["constant", 2+0.1*(t),0.001]
        for _ in range(Anz):
            seed_rand =[]
            ...
            print("## positive Kopplung Testrun")
            ...
            Min_VarAss,Min,TrajectoriesDA,result_List=pbf_min_solver(pbf,pbf_var_dict,"digitalAnnealing",steps,num_MC,cooling_param,seed_rand,seed_gen,visual_inst=False,
                        offset_increase_rate=Offset_increase,save_addinfo=False, save_csv=True,random_start=False)         
            print("### Ergebnisse:")
            test_H1(TrajectoriesDA, TrajectoriesSA)
            ...
            #pbf_min_solver(pbf,pbf_var_dict,"scaAnnealing",200,1,cooling_param,[seed_rand[0]],seed_gen,visual_inst=True,save_csv=True)
            #pbf_min_solver(pbf,pbf_var_dict,"digitalAnnealing_parallel",steps,num_MC,cooling_param,seed_rand,seed_gen,visual_inst=True,offset_increase_rate=Offset_increase,save_csv=False,save_addinfo=True)
            Min_VarAss,Min,TrajectoriesSA,result_List=pbf_min_solver(pbf_neg,pbf_var_dict_neg,"simulatedAnnealing",steps,num_MC,cooling_param,seed_rand,seed_gen,visual_inst=False,
                        offset_increase_rate=Offset_increase,save_addinfo=False, save_csv=True,random_start=False)  
            Min_VarAss,Min,TrajectoriesDA,result_List=pbf_min_solver(pbf_neg,pbf_var_dict_neg,"digitalAnnealing",steps,num_MC,cooling_param,seed_rand,seed_gen,visual_inst=False,
                        offset_increase_rate=Offset_increase,save_addinfo=False, save_csv=True,random_start=False) 
